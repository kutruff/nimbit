...
export function mapOrig<TKey extends Type<unknown, unknown>, TValue extends Type<unknown, unknown>>(
  key: TKey,
  value: TValue
) {
  return new MapTypeOrig<TKey, TValue, Map<TsType<TKey>, TsType<TValue>>>(key, value);
}

export class MapTypeOrig<TKey, TValue, T> extends TypOrig<'map', T> {
  ...
  areEqual(other: Type<unknown, unknown>, cache: ComparisonCache): boolean {
    const otherMap = other as MapTypeOrig<unknown, unknown, unknown>;
    return (
      areEqual(this.value as any, otherMap.value as any, cache) && areEqual(this.key as any, otherMap.key as any, cache)
    );
  }
}
...
export class MapType<TKey, TValue, T> extends Typ<'map', [TKey, TValue], T> {
  ...
  areEqual(other: Typ<unknown, unknown>, cache: ComparisonCache): boolean {
    ...
    return (
      areEqual(this.key as Typ, otherMap.key as Typ, cache) &&
      areEqual(this.value as Typ, otherMap.value as Typ, cache)
    );
  }
}
